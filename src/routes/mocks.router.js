import { Router } from 'express';
import { generateMockUser, generateMockPet, generateMockAdoption } from '../utils/mockDataGenerator.js';
import Users from '../dao/Users.dao.js';
import Pets from '../dao/Pets.dao.js';

const router = Router();

const usersDao = new Users();
const petsDao = new Pets();


router.get('/mockingusers', (req, res) => {
  try {
    const users = Array.from({ length: 50 }, generateMockUser);
    res.status(200).json({ status: 'success', payload: users });
  } catch (error) {
    res.status(500).json({ status: 'error', error: error.message });
  }
});


router.get('/mockingpets', (req, res) => {
  try {
    const pets = Array.from({ length: 50 }, generateMockPet);
    res.status(200).json({ status: 'success', payload: pets });
  } catch (error) {
    res.status(500).json({ status: 'error', error: error.message });
  }
});


router.post('/generateData', async (req, res) => {
  const { users = 0, pets = 0 } = req.body;

  try {
    const mockUsers = Array.from({ length: Number(users) }, generateMockUser);
    const mockPets = Array.from({ length: Number(pets) }, generateMockPet);

    const savedUsers = await Promise.all(mockUsers.map(user => usersDao.save(user)));
    const savedPets = await Promise.all(mockPets.map(pet => petsDao.save(pet)));

    res.status(201).json({
      status: 'success',
      message: `Se generaron ${savedUsers.length} usuarios y ${savedPets.length} mascotas.`,
    });
  } catch (error) {
    res.status(500).json({ status: 'error', error: error.message });
  }
});


router.get('/mockingadoptions', (req, res) => {
  try {
    const adoptions = Array.from({ length: 20 }, generateMockAdoption);
    res.status(200).json({ status: 'success', payload: adoptions });
  } catch (error) {
    res.status(500).json({ status: 'error', error: error.message });
  }
});

export default router;
